		...
		.items([
			S.listItem()
				...
				.child(async (_itemId, { structureContext }) => {
					const client = structureContext.getClient({ apiVersion: API_VERSION });
					...
					const [metadataRows, allPosts] = await Promise.all([
						...
						client.fetch<PostRow[]>(ALL_POSTS_QUERY, {}, { perspective: "raw" }),
					]);
					...
					const groupedItems = metadataRows.map((row) => {
						...
						const ids = expandIdsForDraftLayer(row.translationIds ?? []);
					...
					const orphanItems = orphans.map((p) => {
						...
						const ids = expandIdsForDraftLayer([p._id]);
						return Sb.listItem()
							.title(postLabel(p))
							.id(oid)
							.child(
